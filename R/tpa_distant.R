...
# QUESTION 1            #
#------------------------#
# Installation/mise ÃÂ  jour des librairies
...
library(ROCR)
  ...
  catalogueDB<-DBI::dbGetQuery(con, "SELECT * FROM catalogue")
...
# vehicules_imm <- read.csv("Immatriculations.csv", header = TRUE, sep = ",", dec = ".", stringsAsFactors=T)
immatriculationsDB<-DBI::dbGetQuery(con, 'SELECT immatriculation,marque,nom
                                      ,puissance,nbplaces,nbportes,couleur,
                                      occasion,prix,longueur FROM immatriculations WHERE longueur NOT LIKE \'longueur\'')
...
clients<-DBI::dbGetQuery(con, "SELECT distinct * FROM clients")
...
############ ECRITURE DES DONNEES EN BASE POUR RANDOM FOREST
  marketingDB<-DBI::dbGetQuery(con, "SELECT distinct * FROM marketing")
